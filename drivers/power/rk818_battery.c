...
uint16_t interpolate(int value, uint16_t *table, int size)
{
	uint8_t i;
	uint16_t d;
...
/* Returns (a * b) / c */
int16_t ab_div_c(int16_t a, int16_t b, int16_t c)
{
	bool sign;
	int16_t ans = MAX_INT;
	int32_t tmp;
...
static  int16_t abs_int(int16_t x)
{
	return (x > 0) ? x : -x;
}
...
//	battery_read(di->rk818, CHRG_CTRL_REG2, &chrg_ctrl_reg2, 1);
	...
	usb_ctrl_reg &= (0x01<<7);
	usb_ctrl_reg |= (VLIM_4400MV | ILIM_1400MA);
	
	chrg_ctrl_reg1 &= (0x00);
	chrg_ctrl_reg1 |=(0x01<<7)| (CHRG_VOL4200| CHRG_CUR1200mA);
	...
	if(status == POWER_SUPPLY_STATUS_CHARGING){
		...
		if(!_is_relax_mode( di)){
			...
			if((soc > di->real_soc)&&(di->soc_counter < 10)){
				...
				if((di->soc_counter >= 10)&&(di->real_soc < 100)){
					di->real_soc ++;
					di->soc_counter = 0;
					 //_save_rsoc_nac( di);
				}
