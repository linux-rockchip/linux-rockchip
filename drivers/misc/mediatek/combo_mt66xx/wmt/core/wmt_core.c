...
static INT32 opfunc_hif_conf(P_WMT_OP pWmtOp)
{
    ...
    if (gMtkWmtCtx.wmtInfoBit & WMT_OP_HIF_BIT) {
        ...
        WMT_INFO_FUNC("WMT-CORE: WMT HIF info added\n");
    }
...
static INT32 opfunc_pwr_off(P_WMT_OP pWmtOp)
{
	...
	if (WMT_HIF_UART == gMtkWmtCtx.wmtHifConf.hifType) {
		...
		if (iRet) {
			...
			//DDB: here we need wait untill SDIO card removed.
			INT32 wait_count = 10;
			while(!mtk_wcn_hif_sdio_is_card_removed() && wait_count-- > 0) {
				WMT_INFO_FUNC("wait for card removed.\n");
				osal_msleep(100);
			}
		}
