...
static int rk3188_lcdc_init(struct rk_lcdc_device_driver *dev_drv)
{
	...
	v = 0x00010001;		//rgb/mipi/lvds connect to lcdc0, hdmi connect to  lcdc1
	writel_relaxed(v,RK30_GRF_BASE + GRF_SOC_CON4);
	if(dev_drv->screen0->type == SCREEN_RGB)
	{
		iomux_set(LCDC_DCLK);
		iomux_set(LCDC_DEN);
		iomux_set(LCDC_HSYNC);
		iomux_set(LCDC_VSYNC);
		iomux_set(LCDC_D0);
		iomux_set(LCDC_D1);
		iomux_set(LCDC_D2);
		iomux_set(LCDC_D3);
