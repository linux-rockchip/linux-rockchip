...
#if defined(CONFIG_ARCH_RK32) || defined(CONFIG_ARCH_RK319X)
#define RK32_RGA2_PHYS		0xFFC70000
#define RK32_RGA2_SIZE		0x00010000
#endif
...
static void rga2_del_running_list(void)
{
    ...
    while(!list_empty(&rga2_service.running))
    {
        ...
        if(reg->MMU_len != 0)
        {
            ...
            #if RGA2_TEST
            printk("interrup front=%d back=%d\n", rga2_mmu_buf.front, rga2_mmu_buf.back);
            #endif
        }
