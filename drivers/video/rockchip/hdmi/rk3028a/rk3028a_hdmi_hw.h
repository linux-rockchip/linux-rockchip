#ifndef _RK3028A_HDMI_HW_H
#define _RK3028A_HDMI_HW_H
...
#define SYS_CTRL					0x00
	...
	#define m_PCLK_POL				(1 << 3)
	#define m_REG_CLK_POL			(1 << 4)
	#define m_DIGITAL_RESET			(1 << 5)
	#define m_ANALOG_RESET			(1 << 6)
	
	#define v_INT_POL_HIGH			(1 << 0)
	#define v_INT_POL_LOW			(0 << 0)
	#define v_PWR_ON				(0 << 1)
	#define v_PWR_OFF				(1 << 1)
	#define v_REG_CLK_SOURCE_TMDS	(0 << 2)
	#define v_REG_CLK_SOURCE_SYS	(1 << 2)
	#define v_PCLK_POL_INVERT		(1 << 3)
	#define v_PCLK_POL_NORMAL		(0 << 3)
	#define v_REG_POL_INVERT		(1 << 4)
	#define v_REG_POL_NORMAL		(0 << 4)
	#define v_DIGITAL_RESET(n)		(n << 5)
	#define v_ANALOG_RESET(n)		(n << 6)
...
#define HDMIRdReg(addr)						readl_relaxed(rk3028a_hdmi->regbase + (addr) * 0x04)
#define HDMIWrReg(addr, val)        		writel_relaxed((val), rk3028a_hdmi->regbase + (addr) * 0x04)
#define HDMIMskReg(temp, addr, msk, val)	\
	temp = readl_relaxed(rk3028a_hdmi->regbase + (addr) * 0x04) & (0xFF - (msk)) ; \
	writel_relaxed(temp | ( (val) & (msk) ),  rk3028a_hdmi->regbase + (addr) * 0x04); 
	
extern void rk3028a_hdmi_initial(struct rk3028a_hdmi *rk3028a_hdmi);
extern int rk3028a_hdmi_enable(struct hdmi *hdmi);
extern int rk3028a_hdmi_disable(struct hdmi *hdmi);
extern int rk3028a_hdmi_detect_hotplug(struct hdmi *hdmi);
extern int rk3028a_hdmi_read_edid(struct hdmi *hdmi, int block, unsigned char *buff);
extern int rk3028a_hdmi_insert(struct hdmi *hdmi);
extern int rk3028a_hdmi_removed(struct hdmi *hdmi);
extern int rk3028a_hdmi_config_video(struct hdmi *hdmi, struct hdmi_video *vpara);
extern int rk3028a_hdmi_config_audio(struct hdmi *hdmi, struct hdmi_audio *audio);
extern int rk3028a_hdmi_control_output(struct hdmi *hdmi, int enable);
#endif
