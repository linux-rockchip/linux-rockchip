...
static unsigned long rga_buffer = NULL;
static unsigned int rga_buffer_offset = 0;
//$_rbox_$_modify_$ end
/***************************************************************************
...
#ifdef CONFIG_LCDC_OVERLAY_ENABLE
#include "rga/rga.h"
static void fb_video_scale_by_rga(struct fb_info *info, struct layer_par *par)
{
	struct rk_lcdc_device_driver * dev_drv = (struct rk_lcdc_device_driver * )info->par;
	...
	int ret = 0;
	u32 xsize = 1280, ysize = 720;
	
    memset(&Rga_Request,0x0,sizeof(Rga_Request));

    Rga_Request.src.yrgb_addr =  fix->smem_start;
    Rga_Request.src.uv_addr   =  fix->mmio_start; // RGB格式该值配0
    Rga_Request.src.v_addr    =  Rga_Request.src.uv_addr;
    Rga_Request.src.vir_w = var->xres;  //外框长度
    Rga_Request.src.vir_h = var->yres; //外框高度
    Rga_Request.src.format = RK_FORMAT_YCbCr_420_SP;
    Rga_Request.src.act_w = var->xres;  //内框长度
    Rga_Request.src.act_h = var->yres; //内框高度
    Rga_Request.src.x_offset = 0;
    Rga_Request.src.y_offset = 0;//par->y_offset;
    
    Rga_Request.dst.yrgb_addr = rga_buffer + rga_buffer_offset;
    Rga_Request.dst.uv_addr  = 0;//Rga_Request.dst.yrgb_addr + xsize * ysize;
    Rga_Request.dst.v_addr   = Rga_Request.dst.v_addr;
    Rga_Request.dst.vir_w = xsize;//screen->x_res;   //dst 外框长度
    Rga_Request.dst.vir_h = ysize;//screen->y_res;  //src 外框高度
    Rga_Request.dst.format = RK_FORMAT_RGBX_8888;//RK_FORMAT_YCbCr_420_SP;  
    Rga_Request.clip.xmin = 0;
    Rga_Request.clip.xmax = xsize - 1;
    Rga_Request.clip.ymin = 0;
    Rga_Request.clip.ymax = ysize - 1;
    Rga_Request.dst.act_w = xsize;
    Rga_Request.dst.act_h = ysize;
    ...
    //如果没有使用虚拟地址，以下配置无需打开
    //如果要使用虚拟地址，要求src和dst都是虚拟地址
    //Rga_Request.mmu_info.mmu_en    = 1;
    //Rga_Request.mmu_info.mmu_flag  = ((2 & 0x3) << 4) | 1;

//		printk("\n%s src addr %x dst addr %x\n", __FUNCTION__, Rga_Request.src.yrgb_addr, Rga_Request.dst.yrgb_addr);
	
	ret = rga_ioctl_kernel(&Rga_Request);
    if( ret == 0) {
//	    printk("%s need to scaled by rga\n", __FUNCTION__);
    	par->smem_start = Rga_Request.dst.yrgb_addr;
    	par->y_offset = 0;
    	par->cbr_start = 0;//par->smem_start + xsize * ysize;
    	par->c_offset = 0;
    	par->xvir = xsize;
    	par->yvir = ysize;
    	par->xact = xsize;
    	par->yact = ysize;
    	par->format = XBGR888;
    	if(!rga_buffer_offset)
    		rga_buffer_offset = xsize * ysize * 4;
    	else
    		rga_buffer_offset = 0;
    }
}
...
//$_rbox_$_modify_$ zhengyang modified end
	...
	if(strcmp(info->fix.id, "fb1") == 0 ) {
		if( xvir/xsize >= 8 || yvir/ysize >= 8 ||
			xsize/xvir >= 4 || ysize/yvir >= 4 )
		{
			fb_video_scale_by_rga(info, par);
		}
	}
	#endif
...
static int rk_request_fb_buffer(struct fb_info *fbi,int fb_id)
{
	...
	if (!strcmp(fbi->fix.id,"fb0"))
	{
		...
		fbi->fix.smem_start,fbi->screen_base,fbi->fix.smem_len);
  		...
  		#ifdef CONFIG_LCDC_OVERLAY_ENABLE // alloc ipp buf for video scale
