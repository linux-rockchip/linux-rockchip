...
//				hdl = NULL;
		...
		/* caculate fps */
		u32 cru_tv_sec,cru_tv_usec;
		u32 fps_time;
		do_gettimeofday(&tv);
		cru_tv_sec = tv.tv_sec;
		cru_tv_usec = tv.tv_usec;
		
		if (likely(pre_tv_sec== cru_tv_sec))
			fps_time =  cru_tv_usec - pre_tv_usec ;
		else
			fps_time =  1000000 + cru_tv_usec - pre_tv_usec;
		
		//    printk("fps time:%d\n",fps_time);
		//     printk("fps:%d\n",(1000000/fps_time));
        
		pre_tv_sec = tv.tv_sec;
		pre_tv_usec = tv.tv_usec;
...
static int rk_fb_set_par(struct fb_info *info)
{
	...
	u16 uv_x_off,uv_y_off,uv_y_act;
	u8  is_pic_yuv=0;
//	printk("xres %d yres %d xres_virtual %d yres_virtual %d bits_per_pixel %d\n", 
...
//$_rbox_$_modify_$_end
	...
	if(support_uboot_display())
		return 0;
	main_fbi->var.activate |= FB_ACTIVATE_FORCE;
