...
static patch_info patch_table[] = {
	{ 1828, 0x1200, "rtk8723a", "rtk8723_bt_config", NULL, 0 }, //rtk8723_bt_config
...
#define HARDWARE_ERR_EN 0 //zwx
#define for_test  1
#if for_test //zwx
#else
const unsigned char firmware_array[24404] = {}
#endif
...
//	print_event(urb);
	...
	{
		RTKBT_ERR("btusb_intr_complete HCI_RUNNING = 0");
		return;
	}
...
//	        return;
	...
	{
		RTKBT_ERR("btusb_intr_complete BTUSB_NEXT_RX_URB_SUBMITTING = 0");
		return;
	}
	/*******************************/
	...
	if (urb->status == 0) {
		...
		if (hci_recv_fragment(hdev, HCI_EVENT_PKT,
						urb->transfer_buffer,
						urb->actual_length) < 0) {
			RTKBT_ERR("btusb_intr_complete %s corrupted event packet", hdev->name);
			hdev->stat.err_rx++;
		}
	}
	...
	{
		RTKBT_ERR("btusb_intr_complete BTUSB_INTR_RUNNING = 0");
		return;
	}
	...
	if (err < 0) {
		...
		{
			RTKBT_ERR("btusb_intr_complete %s urb %p failed to resubmit (EPERM)",
						hdev->name, urb);
		}
		else if (err == -ENODEV)
		{
			RTKBT_ERR("btusb_intr_complete %s urb %p failed to resubmit (ENODEV)",
						hdev->name, urb);
		}
		usb_unanchor_urb(urb);
		RTKBT_ERR("btusb_intr_complete usb_unanchor_urb");
	//	dev = urb->dev;
...
static int btusb_submit_intr_urb(struct hci_dev *hdev, gfp_t mem_flags)
{
	...
	RTKBT_DBG("btusb_submit_intr_urb start %s", hdev->name);

	if (!data->intr_ep)
	{
		RTKBT_ERR("btusb_submit_intr_urb error (ENODEV)");
		return -ENODEV;
	}
	...
	if (!urb)
	{
		RTKBT_ERR("btusb_submit_intr_urb error (ENOMEM) (1)");
		return -ENOMEM;
	}
	...
	if (!buf) {
		usb_free_urb(urb);
		RTKBT_ERR("btusb_submit_intr_urb error (ENOMEM) (2)");
		return -ENOMEM;
	}
	...
	RTKBT_DBG("btusb_submit_intr_urb end %s", hdev->name);
...
static void btusb_bulk_complete(struct urb *urb)
{
    ...
    */
	...
	if(!test_bit(BTUSB_NEXT_RX_URB_SUBMITTING, &data->flags))
	{
		RTKBT_ERR("btusb_bulk_complete BTUSB_NEXT_RX_URB_SUBMITTING = 0");
		return;
	}
	/*******************************/
...
static int btusb_open(struct hci_dev *hdev)
{
	...
	RTKBT_DBG("btusb_open set BTUSB_NEXT_RX_URB_SUBMITTING = 1");
	/*******************************/
...
static int btusb_close(struct hci_dev *hdev)
{
	...
	RTKBT_DBG("btusb_close set BTUSB_NEXT_RX_URB_SUBMITTING = 0");
	/*******************************/
				...
				const struct usb_device_id *id)
{
	...
	struct hci_dev *hdev;
	int i, err,flag1,flag2,ap_mdm_id;
...
static int btusb_suspend(struct usb_interface *intf, pm_message_t message)
{
	...
	RTKBT_DBG("btusb_suspend start (2013.08.16.c)");	
	...
	// Test (Realtek)
	clear_bit(BTUSB_NEXT_RX_URB_SUBMITTING, &data->flags);
	RTKBT_DBG("btusb_suspend set BTUSB_NEXT_RX_URB_SUBMITTING = 0");
	/*******************************/
...
static int btusb_resume(struct usb_interface *intf)
{
	...
	RTKBT_DBG("btusb_resume start");  

	if (intf->cur_altsetting->desc.bInterfaceNumber != 0)
	{
		RTKBT_ERR("btusb_resume intf->cur_altsetting->desc.bInterfaceNumber != 0");
		return 0;
	}
	...
	if (--data->suspend_count)
	{
		RTKBT_DBG("btusb_resume data->suspend_count > 0");  
		return 0;
	}
	...
	// Test (Realtek)
	set_bit(BTUSB_NEXT_RX_URB_SUBMITTING, &data->flags);
	RTKBT_DBG("btusb_resume set BTUSB_NEXT_RX_URB_SUBMITTING = 1");
	/*******************************/

	if (test_bit(BTUSB_INTR_RUNNING, &data->flags)) {
		RTKBT_DBG("btusb_resume BTUSB_INTR_RUNNING = 1");  
		err = btusb_submit_intr_urb(hdev, GFP_NOIO);
		if (err < 0) {
			RTKBT_ERR("btusb_resume submit intr. urb error (%d)", -err);
			clear_bit(BTUSB_INTR_RUNNING, &data->flags);
			goto failed;
		}
	}
	...
	if (test_bit(BTUSB_BULK_RUNNING, &data->flags)) {
		...
		if (err < 0) {
			RTKBT_ERR("btusb_resume submit bulk urb error (%d)", -err);
			clear_bit(BTUSB_BULK_RUNNING, &data->flags);
			goto failed;
		}
...
//done:
	...
	RTKBT_DBG("btusb_resume end failed");  
...
int load_firmware(dev_data* dev_entry, uint8_t** buff)
{
	...
	*buff = kzalloc(fw->size, GFP_KERNEL);
	...
	fw_len = fw->size;
#else
	RTKBT_DBG("load_firmware: alloc buff");
	*buff = kzalloc(sizeof(firmware_array), GFP_KERNEL);
	if (NULL == *buff) goto alloc_fail;
	RTKBT_DBG("load_firmware: alloc buff - ok");
	memcpy(*buff, firmware_array, sizeof(firmware_array));
	RTKBT_DBG("load_firmware: memcpy - finish");
	fw_len = sizeof(firmware_array);
	RTKBT_DBG("load_firmware: fw_len = %d", fw_len);
#endif

#if LOAD_CONFIG
	RTKBT_DBG("patch_entry->config_name %s",patch_entry->config_name);
	release_firmware(fw);
...
alloc_fail:
	...
	release_firmware(fw);
#else
	RTKBT_DBG("load_firmware: buff alloc fail");
#endif
fw_fail:
	return fw_len;
}
