...
//		writel(0xffff000f, pll->reg + RK3188_PLL_CON(0));
		writel(0xffff0007, pll->reg + RK3188_PLL_CON(0));
		writel(0xffff00c5, pll->reg + RK3188_PLL_CON(1));
	}
	else {
	writel(clk_set->pllcon0, pll->reg + RK3188_PLL_CON(0));
	writel(clk_set->pllcon1, pll->reg + RK3188_PLL_CON(1));
...
//box
	if (pll->lock) {
		if(!strcmp(__clk_get_name(hw->clk), "clk_cpll")) {
			writel(0, pll->reg + RK3188_PLL_CON(2));
		}
		spin_unlock_irqrestore(pll->lock, flags);
	}
	clk_debug("pll %s dump reg: con0=0x%08x, con1=0x%08x, mode=0x%08x\n",
