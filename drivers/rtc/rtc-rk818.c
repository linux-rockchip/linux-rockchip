...
static int rk818_rtc_probe(struct platform_device *pdev)
{
	...
	rk818_rtc = kzalloc(sizeof(*rk818_rtc), GFP_KERNEL);
	if (rk818_rtc == NULL)
		return -ENOMEM;
	...
	*/
	...
	dev_err(&pdev->dev,"invalid date/time and init time\n");
		rk818_rtc_set_time(&pdev->dev, &tm_def); // 2012-01-01 12:00:00
//		DBG( "set RTC date/time %4d-%02d-%02d(%d) %02d:%02d:%02d\n",1900 + tm_def.tm_year, tm_def.tm_mon + 1, tm_def.tm_mday, tm_def.tm_wday,tm_def.tm_hour, tm_def.tm_min, tm_def.tm_sec);
	...
	ret = request_threaded_irq(per_irq, NULL, rk818_per_irq,
	...
	ret = request_threaded_irq(alm_irq, NULL, rk818_alm_irq,
...
static int __devexit rk818_rtc_remove(struct platform_device *pdev)
{
...
static struct platform_driver rk818_rtc_driver = {
	...
	.remove = __devexit_p(rk818_rtc_remove),
	.driver = {
		.name = "rk818-rtc",
		.pm = &rk818_rtc_pm_ops,
	},
